...
addP :: (Num a, Unbox a, Bin b, BinEq b) => Prof1D b a -> Prof1D b a -> Prof1D b a
addP p p' = over prof (views prof I.hadd p') p

printProf1D :: (Show a, Unbox a, Num a, Bin b, IntervalBin b, Show (BinValue b), Unbox (BinValue b))
            => Prof1D b a -> Text
printProf1D h = T.unlines $ "Total\tTotal\t" <> showBin (V.foldl (<>) mempty . view profData $ h)
